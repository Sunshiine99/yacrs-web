...
class ApiLegacySessionQuestion
{
    ...
    public static function quinfo($user, $config, $mysqli) {
        ...
        if(!isset($_SESSION["yacrs_legacy_key"])) {
            $_SESSION["yacrs_legacy_key"] = DatabaseApiKey::newApiKey($user, $mysqli);
        }
        ...
        $data["questionResponseInfo"]["attributes"]["displayURL"] = "services.php?action=display&key=" . $_SESSION["yacrs_legacy_key"] . "&id=" . $_REQUEST["id"];
